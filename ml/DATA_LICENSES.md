...
| TACO | **Paquete oficial de Zenodo: CC BY 4.0 uniforme** (imágenes + anotaciones COCO) · código MIT | [Zenodo 3587843](https://doi.org/10.5281/zenodo.3587843) · [Repo pedropro/TACO](https://github.com/pedropro/TACO) | **APTO** usando exclusivamente el paquete de Zenodo, cuya licencia declarada cubre el contenido completo (atribución a Proença & Simões). Verificado en la ingesta: el COCO del paquete no contiene metadatos de licencia por imagen (lista `licenses` vacía), así que el plan inicial de filtrar imagen a imagen no aplica a esta distribución; solo aplicaría al flujo `download.py` desde Flickr, que NO se usa |
