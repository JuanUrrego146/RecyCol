...
- **Hallazgo del 07/08/2026 (ML)**: la **carpeta `trash` de Garbage v2** era la causa de la **regresión de acierto de ruta** — su contenido heterogéneo contamina el mapeo hacia `RESIDUAL` y arrastra la métrica de caneca. Tratarla con cuidado en el mapeo de taxonomía (excluirla o re-etiquetarla según decida ML); no reincorporarla tal cual.
- **El criterio de optimización de los modelos es el ACIERTO DE CANECA (ruta), no el top-1 de material.** RNF-008 ya lo ordena («la segunda métrica manda») y desde hoy es explícito: barridos de hiperparámetros, selección de arquitectura y decisiones de datos se comparan por acierto de ruta contra el control; el top-1 de material es métrica secundaria de diagnóstico.
